...
def camera_record():
    ...
    faceCascade = cv2.CascadeClassifier("/home/pi/Desktop/lab1/part2/haarcascade_frontalface_default.xml")
    ...
    while True:
        ...
        if tmp > 0:
            ...
            sense.show_message("alert",text_colour = blue,back_colour = yellow,scroll_speed = 0.05)
            sense.clear()
            sys.exit()
...
initial_temperature = sense.get_temperature()
initial_hum = sense.get_humidity()
...
while True:
    ...
    if ((abs(initial_temperature - tmp) >=1 )or (abs(initial_hum - hum) >= 1)):
        #call the camera stuff
        camera_record()
